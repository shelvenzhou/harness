...
|---|---|---|
| `get` | `key` | Returns value and found flag |
...
When LLM-emitted memory writes start poisoning sessions in practice,
we can layer a confidence model on top: tag every entry with
`confidence ∈ {speculative, verified, user_asserted}` and `provenance`
(threadId / eventId / sourceTool / createdAt), assigned by the runtime
based on the source event (LLM `memory.set` → `speculative`, trusted
adapter ingest → `user_asserted`, verifier subagent → `verified`).
Projection renders entries as `<memory confidence="…">…</memory>` so
the model weights speculative claims accordingly. An async verifier
sweep promotes / demotes / deletes speculative entries on cadence.

Deferred until a concrete poisoning incident motivates it. The current
KV path with `pinned` is enough for the cases we hit today.
